...
namespace SourceEstimators
{
  namespace FishTagXKF
  {
    ...
    uint8_t
    LeastSquares::LeastSquaresEstimate(DUNE::Math::Matrix receiverPositions, DUNE::Math::Matrix RDOA, DUNE::Math::Matrix tagDepth, bool newMeasurement[3])
    {
        uint8_t isSuccess = 0;
        ...
        double d1 = -RDOA(2,0);
        double d2 = RDOA(1,0);
        double depth = -(newMeasurement[0]*tagDepth(0,0) + newMeasurement[1]*tagDepth(1,0) + newMeasurement[2]*tagDepth(2,0))/(newMeasurement[0] + newMeasurement[1] + newMeasurement[2]);
