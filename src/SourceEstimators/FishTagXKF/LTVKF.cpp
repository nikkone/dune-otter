...
void LTVKF::constructCandR(DUNE::Math::Matrix receiverPositions, DUNE::Math::Matrix RDOA, DUNE::Math::Matrix tagDepth, bool validCombination[3], double m_dr)
    {
        ...
        double depth = (m23*(tagDepth(1,0) + tagDepth(2,0))/2 + m31*(tagDepth(2,0) + tagDepth(0,0))/2)/(m23 + m31);

        Ck.put(temp_ind, 0, C.row(2));
        Rk(temp_ind, temp_ind) = rz_cov;
        yk(temp_ind, 0) = depth/2;
        ykest(temp_ind, 0) = Yest(temp_ind, 0);
