...
namespace SourceEstimators
{
  namespace FishTagXKF
  {
    ...
    void XKF::update(DUNE::Math::Matrix receiverPositions, DUNE::Math::Matrix RDOA, DUNE::Math::Matrix tagDepth, bool newMeasurement[3], bool validCombination[3]) {

        if(validCombination[1] && validCombination[2])
        {
          bool isSuccess = stage1.LeastSquaresEstimate(receiverPositions, RDOA, tagDepth, newMeasurement);
        ...
        if(stage3.active)
        {
          stage3.constructCandR(receiverPositions, RDOA, tagDepth, newMeasurement, validCombination, stage2.xHat);
          stage3.update();
        }
    }
