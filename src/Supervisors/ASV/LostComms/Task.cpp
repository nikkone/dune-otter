...
//***************************************************************************
// Author: Pedro Calado (Modified by Nikolai Lauvås from AUV task)          *
//***************************************************************************
  ...
  namespace ASV
  {
    namespace LostComms
    {
      ...
      struct Arguments
      {
        ...
        //! Heartbeat timeout.
        float timeout;
        //! Require Idle to start plan
        bool requireIsIdle;
        //! Require Bas outcome to start plan
        bool requireBadOutcome;
      };
      ...
      struct Task: public DUNE::Tasks::Periodic
      {
          ...
          m_lcs(STATE_NOT_MET)
        {
          ...
          param("Require Idle", m_args.requireIsIdle)
          .defaultValue("true")
          .description("Name for the lost communciations plan");

          param("Require Bad Outcome", m_args.requireBadOutcome)
          .defaultValue("true")
          .description("Name for the lost communciations plan");
