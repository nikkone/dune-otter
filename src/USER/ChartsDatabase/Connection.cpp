...
namespace DUNE
{
  namespace ChartsDatabase
  {
    ChartsDBConnection::ChartsDBConnection(const char* path, int flags):Connection(path, flags)  {
      ...
      Database::Statement* iterator_stmt = new Database::Statement(c_stmt.c_str(), *this);
    ...
    int ChartsDBConnection::checkTransectLanding(double startLat, double startLon, double endLat, double endLon, std::string table, bool useSpatialIndex) {
      Database::Statement* iterator_stmt;
      ...
      if(useSpatialIndex) {
        ...
        iterator_stmt = new Database::Statement(c_stmt.c_str(), *this); 
...
/*
    ...
    double ChartsDBConnection::checkPlanMinDepth(const IMC::PlanSpecification* plan, double currentLat, double curentLon, std::string table) {   
    ...
    bool ChartsDBConnection::checkPlanLanding(const IMC::PlanSpecification* plan, double currentLat, double curentLon, std::string table) {
    ...
    std::string ChartsDBConnection::planToWKT(const IMC::PlanSpecification* plan, double currentLat, double curentLon) {
      ...
      for (std::vector<IMC::PlanManeuver*>::const_iterator itr = plan->maneuvers.begin(); itr != plan->maneuvers.end(); ++itr)
      {
...
///////////////////////////////////////////////////////////////
/*
    ChartsDBConnection::DBTree::DBTree(ChartsDatabase::ChartsDBConnection* dbcon)  : m_con(dbcon){
    }
    bool ChartsDBConnection::DBTree::resetTree(std::string tableName) {
    ...
    bool ChartsDBConnection::DBTree::createTree(std::string dbTreeName) {
    ...
    bool ChartsDBConnection::DBTree::deleteTree(std::string dbTreeName) {
    ...
    bool ChartsDBConnection::DBTree::insertNode(std::string tableName, unsigned ParentID,double lat, double lon, unsigned ID) {
      std::string IDStr;
      if(ID==0) {
        IDStr="NULL";
      } else {
    ...
    std::pair<double, double> ChartsDBConnection::DBTree::getNodeLocation(std::string tableName, unsigned parentID) {
      std::string c_stmt2 = "select X(geom), Y(geom) from (select geom from " + tableName + " where ID="+std::to_string(parentID)+")";
