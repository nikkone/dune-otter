...
namespace ENCGIS
{
  ...
  bool DBconnection::isPointInLayer(double X, double Y, std::string table, bool useSpatialIndex) {
    ...
    if(sqlite3_step(m_handle) == SQLITE_ROW) {
      if (sqlite3_column_type(m_handle, 0) != SQLITE_INTEGER) {
        sqlite3_finalize(m_handle);
  ...
  bool DBconnection::runQuery(const std::string &sql_stmt) {
      ...
      if(sqlite3_exec(db, sql_stmt.c_str(), callback, 0, &zErrMsg)!=SQLITE_OK ){
        if(zErrMsg == NULL) {
          Error("SQL error: %s\n", "Received NULL as error message, but SQLITE_OK not received. Does the databasefile exist?");
        } else {
          Error("SQL error: %s\n", zErrMsg);
        }
  ...
  bool DBconnection::getExtent(std::string layer, double &minX, double &minY, double &maxX, double &maxY) {
    ...
    if (sqlite3_prepare_v2(db, query.c_str(), query.length(), &m_handle, 0) == SQLITE_OK) {
      if(sqlite3_step(m_handle) == SQLITE_ROW) {
        minX = sqlite3_column_double(m_handle, 0);
        minY = sqlite3_column_double(m_handle, 1);
        maxX = sqlite3_column_double(m_handle, 2);
        maxY = sqlite3_column_double(m_handle, 3);
        sqlite3_finalize(m_handle);
        return true;
      }
    }
    ...
    return false;
      ...
      bool DBconnection::findClosestSafePointUTM(double &X, double &Y, double shiftDistance, double MBROffset) {
        ...
        int rc = 0;
        if (( rc = sqlite3_prepare_v2(db, x.c_str(), x.length(), &db_handle, 0)) != SQLITE_OK) {
            sqlite3_finalize(db_handle);
            std::cout << "findClosestSafePointUTM error: First Prepare not ok. Code: " << rc << std::endl;
            return false;
        }

        // Execute
        if((rc = sqlite3_step(db_handle)) == SQLITE_ROW) {
          if(sqlite3_column_int(db_handle, 0)) {
            // Returned point same as given point, so no need to shift
            sqlite3_finalize(db_handle);
            return true;
          } else if(shiftDistance <= 0.0) {
            X = sqlite3_column_double(db_handle, 1);
            Y = sqlite3_column_double(db_handle, 2);
            sqlite3_finalize(db_handle);
            return true;
          } else {
            // Returned point on intersection line between innavigable and navigable, therefore:
            // Read intersection point
            double tempX = sqlite3_column_double(db_handle, 1);
            double tempY = sqlite3_column_double(db_handle, 2);
            ...
            {
              sqlite3_finalize(db_handle);
              std::cout << "findClosestSafePointUTM error: Second Prepare not ok. Code: " << rc << std::endl;
              return false;
            }
            ...
            if((rc = sqlite3_step(db_handle)) == SQLITE_ROW) {
              ...
              sqlite3_finalize(db_handle);
              std::cout << "findClosestSafePointUTM error: Second step not ok. Code: " << rc << std::endl;
              return false;
            }
          }
      }
        ...
        sqlite3_finalize(db_handle);
        std::cout << "findClosestSafePointUTM error: First step failed. Code: " << rc << std::endl;
        return false;
      }
    ...
    double DBconnection::distanceToLayerUTM(const double x, const double y, const double maxDist, std::string layer) {
      ...
      ")";
        ...
        // Execute
        if(sqlite3_step(m_handle) == SQLITE_ROW) {
          double minDist = sqlite3_column_double(m_handle, 0);
    ...
    bool DBconnection::distanceToLayerWithinUTM(const double x, const double y, const double limit, std::string layer) {
      ...
      ") where d=1 limit 1";
        ...
        // Execute
        if(sqlite3_step(m_handle) == SQLITE_ROW) {
          int within = sqlite3_column_int(m_handle, 0);
          if(within) {
            sqlite3_finalize(m_handle);
            return true;
          }
