...
bool LTVKF<T>::constructCandR(const TagBuffer *tagBuffer, const Eigen::Matrix<T, Eigen::Dynamic, 1> RDOA, const std::vector<std::pair<uint32_t, uint32_t>> RDOAcombinations, T dm)
    {
      ...
      OFP::KalmanFilterDynamic<T, 6>::C.resize(RDOA.rows()+1,OFP::KalmanFilterDynamic<T, 6>::nx);
      ...
      //OFP::KalmanFilterDynamic<T, 6>::C.row(used) = OFP::KalmanFilterDynamic<T, 6>::C.row(used).Zero(1,OFP::KalmanFilterDynamic<T, 6>::nx);
      //OFP::KalmanFilterDynamic<T, 6>::C.bottomRightCorner(1, 1) << 0.5;
      OFP::KalmanFilterDynamic<T, 6>::C.row(used).bottomLeftCorner(1, 3) << 0,0,0.5;
      ...
      OFP::KalmanFilterDynamic<T, 6>::yk = z/2 + dm*d; // New measurement vector

      OFP::KalmanFilterDynamic<T, 6>::ykest = OFP::KalmanFilterDynamic<T, 6>::C*OFP::KalmanFilterDynamic<T, 6>::xHat; // (15) from paper
      // Compute R matrix
      OFP::KalmanFilterDynamic<T, 6>::R = OFP::KalmanFilterDynamic<T, 6>::R.Zero(RDOA.rows()+1,RDOA.rows()+1);
      for(int i=0;i<used;i++) {
        OFP::KalmanFilterDynamic<T, 6>::R(i, i) = 0.5*pow(2*rr_cov,2) + pow(d(i) + dm,2)*2*rr_cov;
        for(int j=0;j<used;j++) {
          if(j!=i) {
            OFP::KalmanFilterDynamic<T, 6>::R(i,j) = 0.5*pow(rr_cov,2) + (d(i)*d(j) + dm*(d(i)+ d(j)) + dm*dm)*rr_cov;
            OFP::KalmanFilterDynamic<T, 6>::R(j,i) = 0.5*pow(rr_cov,2) + (d(j)*d(i) + dm*(d(j)+ d(i)) + dm*dm)*rr_cov;
          }
        }
      }
