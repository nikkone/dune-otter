...
{
  template <class T>
  void MultipleReceiverXKFVelocity<T>::initialize(const Eigen::Matrix<T, c_states, c_states> &A_inn,
  ...
  template <class T>
  void MultipleReceiverXKFVelocity<T>::predict() {
    xkf.predict();
  }

  template <class T>
  std::tuple<T, T, T> MultipleReceiverXKFVelocity<T>::getEstimate() const{
    /*
  ...
  template <class T>
  void MultipleReceiverXKFVelocity<T>::setTDOACovariance(T TDOACovariance) {
    rr_cov = TDOACovariance;
    xkf.setDiagonalCovarianceR(TDOACovariance);
  }

  template <class T>
  void MultipleReceiverXKFVelocity<T>::setDepthCovariance(T depthCovariance) {
    rz_cov = depthCovariance;
    xkf.setVarianceRZ(depthCovariance);
  }

  template <class T>
  bool MultipleReceiverXKFVelocity<T>::update(TagBuffer *tagBuffer) {
    //std::cout << "Entering MultipleReceiverXKFVelocity<T>::update" << std::endl;
...
template <class T>
  bool MultipleReceiverXKFVelocity<T>::checkTime(T transmissionFirstTime, T currentTime) const {
